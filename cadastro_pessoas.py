...
def add_pessoa():
    ...
    while True:
        ...
        if re.fullmatch("(\d\d\/\d\d\/\d\d\d\d)", data_nascimento):
            a = int(data_nascimento[3:5]) < 13
            b = int(data_nascimento[:2]) < 32
            c = int(data_nascimento[6:]) < today.year + 1
            d = int(data_nascimento[3:5]) <= today.month
            e = int(data_nascimento[:2]) <= today.day
    ...
    r = re.compile(r'^[\w-]+@(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}$') # https://pt.stackoverflow.com/questions/365442/fun%C3%A7%C3%A3o-de-valida%C3%A7%C3%A3o-de-email
    while True:
        email = input('E-mail: ')
        if r.match(email):
            break
